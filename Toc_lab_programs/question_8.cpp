...
enum states{q0_starting,q1,q2_finish,Rejected};
int main()
{
    ...
    if (NFA_endingwith_01(input)) {cout<<"String is accepted"<<endl;}
...
bool NFA_endingwith_01(char w[])
{
    ...
    for(int i = 0;i<strlen(w);i++)
    {
        ...
        else
        {
            ...
            else if(state == q1 && check_value == "1"){state = q2_finish;}
            else if(state == q2_finish && check_value == "0"){state = q1;}
            else if(state == q2_finish && check_value == "1"){state = q0_starting;}
        }
    }
